...
@section('page-title', 'Edit Plan Feature')
    ...
    <div class="row">
        <div class="col-md-8 offset-md-2">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.pricing-plan-features.update', $pricingPlanFeature) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            ...
                            <select name="pricing_plan_id" id="pricing_plan_id" class="form-control @error('pricing_plan_id') is-invalid @enderror" required>
                                ...
                                @foreach($plans as $plan)
                                    <option value="{{ $plan->id }}" {{ old('pricing_plan_id', $pricingPlanFeature->pricing_plan_id) == $plan->id ? 'selected' : '' }}>{{ $plan->name }}</option>
                                @endforeach
                            </select>
                            @error('pricing_plan_id')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        @include('admin.partials.crud-form', ['model' => $pricingPlanFeature])
