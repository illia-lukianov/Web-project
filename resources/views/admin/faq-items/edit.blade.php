        ...
        <div class="col-md-8 offset-md-2">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Edit FAQ Item</h3>
                </div>
                <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="faq_section_id" id="faq_section_id" class="form-control @error('faq_section_id') is-invalid @enderror" required>
                                ...
                                @foreach($sections as $sec)
                                    <option value="{{ $sec->id }}" {{ old('faq_section_id', $faqItem->faq_section_id) == $sec->id ? 'selected' : '' }}>{{ $sec->title }}</option>
                                @endforeach
                            </select>
                            @error('faq_section_id')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        @include('admin.partials.crud-form', ['model' => $faqItem])
